...
pub fn org_query_full_tree<T>(filter_parent_id: i32) -> Vec<OrgResponse> {
    ...
    let predicate =
        crate::model::diesel::dolphin::dolphin_schema::org::parent_id.eq(filter_parent_id);
    let root_menus = org
        .filter(&predicate)
        .order(sort.asc())
        .load::<Org>(&mut get_conn())
...
pub fn org_query_tree<T>(request: &Json<OrgRequest>) -> PaginationResponse<Vec<OrgResponse>> {
    ...
    let predicate =
        crate::model::diesel::dolphin::dolphin_schema::org::parent_id.eq(request.parentId);
    let query = org
        .filter(&predicate)
        .paginate( request.pageNum)
        .per_page(request.pageSize);
    let query_result: QueryResult<(Vec<_>, i64)> =
        query.load_and_count_pages(&mut connection);
    let menu_responses = find_sub_org_cte_impl(&query_result.as_ref().unwrap().0);
    let total = 200;
    let page_result =
        map_pagination_from_list(menu_responses, request.pageNum, request.pageSize, total);
    return page_result;
}
...
pub fn org_edit(request: &Json<PasswordRequest>) -> content::RawJson<String> {
    ...
    let predicate =
        crate::model::diesel::dolphin::dolphin_schema::admin_users::phone.eq(request_user_name);
