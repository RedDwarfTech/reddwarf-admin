...
pub fn menu_query_full_tree<T>(filter_parent_id: i32) -> Vec<MenuResponse> {
    ...
    let predicate = crate::model::diesel::dolphin::dolphin_schema::menu_resource::parent_id
        .eq(filter_parent_id);
    let root_menus = menu_resource
        .filter(&predicate)
        .order(sort.asc())
...
pub fn menu_query_tree<T>(request: &Json<MenuRequest>) -> PaginationResponse<Vec<MenuResponse>> {
    ...
    let predicate = crate::model::diesel::dolphin::dolphin_schema::menu_resource::parent_id
        .eq(request.parentId);
    let query = menu_resource
        .filter(&predicate)
        .paginate(request.pageNum, false)
        .per_page(request.pageSize);
    let query_result: QueryResult<(Vec<_>, i64)> =
        query.load_and_count_pages::<MenuResource>(&mut get_conn());
...
pub fn menu_edit(request: &Json<UpdateMenuRequest>) -> content::RawJson<String> {
    ...
    let update_records = diesel::update(menu_resource.filter(predicate))
        .set((
            sort.eq(request.sort),
            path.eq(request.path.to_string()),
            component.eq(request.component.to_owned()),
        ))
