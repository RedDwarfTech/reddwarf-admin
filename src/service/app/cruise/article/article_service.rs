...
type QueryType<'a> = BoxedSelectStatement<
    ...
    (
        ...
        diesel::sql_types::Nullable<diesel::pg::sql_types::Timestamptz>,
        diesel::sql_types::BigInt,
        diesel::sql_types::Nullable<diesel::sql_types::Text>,
    ...
    FromClause<crate::model::diesel::dolphin::dolphin_schema::article::table>,
    diesel::pg::Pg,
>;

pub fn article_query<T>(
    request: &Json<ArticleRequest>,
...
pub fn append_channel_name(
    ...
    for article in articles {
        let channel_name = channels
            .iter()
            .filter(|channel| channel.id == article.sub_source_id)
...
pub fn article_detail_query(filter_article_id: i64) -> ArticleResponse {
    ...
    let query = article
        .filter(crate::model::diesel::dolphin::dolphin_schema::article::id.eq(filter_article_id))
        .limit(1)
        .load::<Article>(&mut get_conn())
        .expect("query article id failed");
    let article_result: Article = take(query, 0).unwrap();
    let contents = article_content
        .filter(
            crate::model::diesel::dolphin::dolphin_schema::article_content::article_id
                .eq(filter_article_id),
        )
        .limit(1)
    ...
    let channel_predicate = crate::model::diesel::dolphin::dolphin_schema::rss_sub_source::dsl::id
        .eq(&article_result.sub_source_id);
    // https://stackoverflow.com/questions/46297867/how-do-i-get-an-optiont-instead-of-an-optionvect-from-a-diesel-query-which
    let channel = rss_sub_source
        .filter(channel_predicate)
...
pub fn get_article_count_by_channel_id(req_channel_id: &i64) -> i64 {
    use crate::model::diesel::dolphin::dolphin_schema::article::dsl::*;
    let predicate =
        crate::model::diesel::dolphin::dolphin_schema::article::sub_source_id.eq(req_channel_id);
    let query = article.filter(predicate);
