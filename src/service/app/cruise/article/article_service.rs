use diesel::{delete, ExpressionMethods, QueryDsl, QueryResult};
...
use crate::model::diesel::dolphin::dolphin_schema::article::dsl::article;
...
pub fn article_query<T>(request: &Json<ArticleRequest>) -> PaginationResponse<Vec<Article>> {
    ...
    let query = article.filter(editor_pick.eq(1))
