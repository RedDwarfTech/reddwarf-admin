...
pub fn channel_query<T>(request: ChannelRequest, _login_user_info: LoginUserInfo) -> PaginationResponse<Vec<RssSubSource>> {
    ...
    if let Some(channel_sub_name) = request.sub_name {
        if !channel_sub_name.trim().is_empty() {
            query = query.filter(channel_table::sub_name.like(format!("{}{}{}", "%", channel_sub_name.as_str().trim(), "%")));
        }
    }
    ...
    let query_result: QueryResult<(Vec<_>, i64, i64)> = query.load_and_count_pages_total::<RssSubSource>(&mut get_conn());
